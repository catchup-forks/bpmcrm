            ...
            <div class="container">
                <div class="row">
                    <div class="col">
                        ...
                        <br>
                        <div>Created:<span
                                    class="font-weight-bold"> {{$group->created_at->format('m/j/Y g:i A')}}</span></div>
                        <div>Updated: <span
                                    class="font-weight-bold"> {{$group->updated_at->format('m/j/Y g:i A')}} </span>
                        </div>
                        <br>
                    </div>
                ...
                @if ($group->groupMembers->count() > 0)
                    <table class="table table-hover vuetable">
                        ...
                        @foreach ($group->groupMembers as $groupMember)
                            @if ($groupMember->member_type == \app\Models\User::class)
                                @php ($user = $groupMember->member)
                                <tr>
                                    <td scope="row"><img
                                                src="{{ asset('img/avatar_placeholder_small.png') }}"/> {{$user->getFullName()}}
                                    </td>
                                    <td>{{$user->email}}</td>
                                    @if ($group->status == 'ACTIVE')
